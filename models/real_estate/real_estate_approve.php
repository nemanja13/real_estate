<?php
    ...
    if(isset($_POST["submit"]) && isset($_POST["idRealEstate"])){
        $idRealEstate=$_POST["idRealEstate"];
        $query="UPDATE real_estate SET approved=1 WHERE idRealEstate=?";
        $prepare=$connection->prepare($query);
        try{
            if($prepare->execute([$idRealEstate])){
                $code=204;
            }
        }
